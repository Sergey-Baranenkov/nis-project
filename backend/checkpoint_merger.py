...
from diffusers import DiffusionPipeline, StableDiffusionControlNetPipeline, __version__
...
class CheckpointMergerPipeline(DiffusionPipeline):
    ...
    def merge(self, pretrained_model_name_or_path_list: List[Union[str, os.PathLike]], **kwargs):
        ...
        final_pipe = DiffusionPipeline.from_pretrained(
            cached_folders[0], controlnet=ControlNetModel.from_pretrained("lllyasviel/sd-controlnet-canny", torch_dtype=torch.float16), torch_dtype=torch_dtype, device_map=device_map, custom_pipeline="lalala.py"
        )
                ...
                for key in theta_0.keys():
                    if theta_2:
                        try:
                            ...
                            ones = torch.zeros(theta_1[key].size()[0], 9, theta_1[key].size()[2], theta_1[key].size()[3])
                            ones[:, :theta_1[key].size()[1], :, :] = theta_1[key]
                            theta_1[key] = ones
                            ones = torch.zeros(theta_2[key].size()[0], 9, theta_2[key].size()[2], theta_2[key].size()[3])
                            ones[:, :theta_2[key].size()[1], :, :] = theta_2[key]
                            theta_2[key] = ones
                            theta_0[key] = theta_func(theta_0[key], theta_1[key], theta_2[key], alpha)
                    else:
